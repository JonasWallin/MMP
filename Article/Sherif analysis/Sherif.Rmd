...
```
We then fit a baseline model to these data. Given the model’s complexity, convergence to a global optimum isn’t guaranteed, and in fact the initial fit suggests a pattern that contradicts our expectations: the estimated group‐level variance actually decreases rather than increases over time.
...
Next, we estimate the group‐level and individual‐level parameters separately—using each set of estimates as starting values for the full model refit. After refitting, the group‐level variance now follows the upward trajectory over time seen in the data.
```{r refit, echo=FALSE}
param.group <- c(3,0.1,0.1,0.1,0.1) # first parameter is mean of the group, rest is just random
param_list <- estimate_group_effects(GP.h$covariances, GP.h, param.group)
param <- c(0,0,0,0,0) #meauserment error
param_list <- estimate_error_indv_effects(param_list, GP.h, param)
par <- listToParam(param_list,GP.h$object)
GP.h2 <- ce(y ~ 1, 
           ~ 1 | person, 
           ~ 1 | group, 
           emergence = ~ 1, 
           ...
           data = sherifdat,
           param = par)
Covs.fit <- get.Cov(GP.h2$covariances,GP.h2$object)
print('Variance for team effect over time')
print(diag(Covs.fit$SigmaT))
```
...
CEI2.h <- ce(y ~ 1+time, 
             ...
             data = sherifdat)
param.group <- c(3,0.1,0.1,0.1,0.1) # first parameter is mean of the group, rest is just random
param_list <- estimate_group_effects(CEI2.h$covariances, CEI2.h, param.group)
param <- c(0,0,0,0) #meauserment error
param_list <- estimate_error_indv_effects(param_list, CEI2.h, param)
par <- listToParam(param_list,CEI2.h$object)
CEI2.h2 <- ce(y ~ 1+time, 
             ~ 1 | person, 
             ~ 1 | group, 
             emergence = ~ -1 + time, 
...
GP <- ce(y ~ 1+time, 
         ...
         data = sherifdat)
param <- c(0,0,0,0,0) #meauserment error
param_list <- estimate_error_indv_effects(GP$covariances, GP, param)
par <- listToParam(param_list,GP$object)
GP <- ce(y ~ 1+time, 
         ~ 1 | person, 
         ~ 1 + time | group, 
         emergence = ~ 1, 
...
```
The random effect on the group level is much worse, compared to fitting Gaussian processes on two levels. The random effect on individual level is basically equivalent to the Gaussian process.
```{r AIC, echo=FALSE}
