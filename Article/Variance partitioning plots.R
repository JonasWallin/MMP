...
t <- 0:3
VeY <- rep(0.06,4)
VP <- c(3.39,0.3,0.03,0.01)
VG <- c(4.46,2.15,1.55,1.39)
...
varplot2 <- ggarrange(pvar, pperc, 
                      ...
                      common.legend = T,
                      legend="bottom")
#ggsave("sherif_varianceplot2.pdf",varplot2)  

#prop <- dat %>% 
#  pivot_longer(c(PVeY,PVP,PVG,PVY),names_to = "var",values_to = "percent") 

#freq <- dat %>% 
#  pivot_longer(c(VeY,VP,VG,VY),names_to = "var",values_to = "value") 

prop <- dat %>% 
    pivot_longer(c(PVeY,PVP,PVG),names_to = "var",values_to = "percent") 
  
freq <- dat %>% 
  pivot_longer(c(VeY,VP,VG),names_to = "var",values_to = "value") 
  
all <- cbind(freq[,c("t","var","value")],prop[,c("percent")])
all$tot <- rep(dat$tot,each=3)

variance_plot <- all %>% 
  ggplot(aes(x=t,y=value))+
  geom_point(aes(size=percent,col=var),alpha=0.5,show.legend = F)+
  geom_line(aes(col=var),alpha=0.5)+
  geom_line(aes(x=t,y=tot),alpha=0.5)+
  theme_bw()+
  ylab("Variance") +
  xlab("Time") +
  scale_size(range = c(.7, 10),
             name="Proportion of total variance")+
  scale_color_discrete(name="Variance",labels = c("VeY" = TeX('$\\V[e^Y_{tij}]$'),"VG" = TeX('$\\V[G_{tij}]$'),"VP" = TeX('$\\V[P_{tij}]$'),"VY" = TeX('$\\V[Y_{tij}]$')))
# theme(legend.position = "none")
variance_plot
#ggsave("sherif_varianceplot.pdf",variance_plot)  
...
variance_plot <- all %>% 
 ...
 # theme(legend.position = "none")
variance_plot
#ggsave("cohesion_varianceplot.pdf",variance_plot)  
